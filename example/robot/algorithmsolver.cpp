...
QVector<QPair<int, int> > AlgorithmSolver::solve()
{
    Node node;
    node.parent = NULL;
    ...
    QVector<QPair<int, int> > p = QVector<QPair<int, int> >();
    for (int i = 0; i < bottomNode->path.size(); i++)
        p.push_back(bottomNode->path.at(i).first);
    return p;




    bottomBoundary = bottomNode->mark;
//    Node* best = findBetter(bottomNode);

    QVector<QPair<int, int> > path =  QVector<QPair<int, int> > ();

    for (int i = 0; i < contour.size(); i++) {
        bool flag = false;
        for (int j = 0; j < path.size(); j++) {
            if (contour.at(i).first == path.at(j).first
                    && contour.at(i).second == path.at(j).second)
                flag = true;
        }
        if (!flag) {
            path.append(QPair<int, int> (contour.at(i).first,
                                         contour.at(i).second));
        }
    }

    for (int i = 0; i < bottomNode->path.size(); i++)
        std::cout << bottomNode->path.at(i).first.first << "; "
                  << bottomNode->path.at(i).first.second << " -> ";
    std::cout << "\n";

    std::cout << "mark2:" << bottomBoundary << "\n";
...
QPair<QPair<int, int>, int> AlgorithmSolver::getChain(QVector<QPair<QPair<int, int>, int> > path,
                                                      QVector<QPair<QPair<int, int>, int> > chains)
{
/*    for (int i = 0; i < chains.size(); i++) {
        bool repeat = false;
        for (int j = 0; j < path.size(); j++)
            if (chains.at(i).first.second == path.at(j).first.first)
                repeat = true;
        if (!repeat)
            return chains.at(i);
    }*/
...
bool AlgorithmSolver::isCycle(QPair<int, int> chain, QVector<QPair<QPair<int, int>, int> > path)
{
    QPair<int, int> tmpChain;
    tmpChain.first = chain.first;
    tmpChain.second = chain.second;
    for (int i = 0; i < path.size(); i++) {
        for (int j = 0; j < path.size(); j++) {
            bool match = false;
            if (tmpChain.second == path.at(j).first.first) {
                tmpChain.first = path.at(j).first.first;
                tmpChain.second = path.at(j).first.second;
                match = true;
            }

            if (match && tmpChain.first == chain.first && tmpChain.second == chain.second)
                return true;
        }
    }
...
Node* AlgorithmSolver::divaricate(Node* node)
{
    ...
    if (m != NULL) {
        ...
        if (node->mark < bottomBoundary){
            contour.clear();

            for (int i = 0; i < node->path.size(); i++) {
                bool flag = false;
                for (int j = 0; j < contour.size(); j++) {
                    if (node->path.at(i).first.first == contour.at(j).first
                            && node->path.at(i).first.second == contour.at(j).second)
                        flag = true;
                }
                if (!flag) {
                    contour.append(QPair<int, int> (node->path.at(i).first.first,
                                                    node->path.at(i).first.second));
                }
            }
        }
    ...
    if (isCycle(chains.at(0).first, node->left->path)) {
        std::cout << "asdfasd" << "\n";
        node->left->mark = std::numeric_limits<double>::infinity();
    }
...
Node* AlgorithmSolver::findBetter(Node* node)
{
    getchar();
    if (node->parent == NULL)
        return NULL;

    Node* neighbour = (node->parent->left == node)?(node->parent->right):(node->parent->left);

    Node* better = NULL;
    if (neighbour->left == NULL && neighbour->right == NULL && node->mark < bottomBoundary) {
        better = divaricate(neighbour);
        findBetter(better);
    }

    Node* otherBetter = findBetter(node->parent);

    if (otherBetter != NULL && better != NULL) {
        Node* best = (better->mark > otherBetter->mark)?(better):(otherBetter);
        bottomBoundary = best->mark;
        contour.clear();
        for (int i = 0; i < best->path.size(); i++)
            contour.append(QPair<int, int> (best->path.at(i).first.first,
                                            best->path.at(i).first.second));

        return best;
    } else if (otherBetter == NULL && better != NULL && better->mark < bottomBoundary) {
        bottomBoundary = better->mark;
        contour.clear();
        for (int i = 0; i < better->path.size(); i++)
            contour.append(QPair<int, int> (better->path.at(i).first.first,
                                            better->path.at(i).first.second));
        return better;
    } else if (better == NULL && otherBetter != NULL && otherBetter->mark < bottomBoundary) {
        bottomBoundary = otherBetter->mark;
        contour.clear();
        for (int i = 0; i < otherBetter->path.size(); i++)
            contour.append(QPair<int, int> (otherBetter->path.at(i).first.first,
                                            otherBetter->path.at(i).first.second));
        return otherBetter;
    } else
        return NULL;
}
